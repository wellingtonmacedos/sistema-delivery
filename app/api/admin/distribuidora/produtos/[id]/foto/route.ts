...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!produtoId) return Response.json({ error: 'produto inválido' }, { status: 400 })
  ...
  const name = `produto_${produtoId}_${Date.now()}.${ext}`
  const full = path.join(dir, name)
  fs.writeFileSync(full, bytes)
  const url = `/uploads/produtos/${name}`
  await prisma.produto.update({ where: { id: produtoId }, data: { fotoUrl: url } })
  return Response.json({ ok: true, fotoUrl: url })
}
